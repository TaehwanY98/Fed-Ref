...
class FedRef(flwr.server.strategy.FedAvg):
    ...
    def evaluate(self, server_round: int, parameters) -> Optional[Tuple[float, Dict[str, flwr.common.Scalar]]]:
        ...
        # 파일 저장 경로 처리
        os.makedirs(os.path.join(self.args.result_path, self.args.mode, f"degrade{self.args.degrade}", self.args.clientMode), exist_ok=True)
        csv_path = os.path.join(self.args.result_path, self.args.mode, f"degrade{self.args.degrade}", self.args.clientMode, f'{self.args.mode}_{self.args.type}_lda{self.args.lda*10}_p{self.args.prime}.csv')
